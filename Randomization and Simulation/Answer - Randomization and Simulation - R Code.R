...
# Taking a sample with n = 20 and p = 0.5 (given on the question)
...
# Answer to the question No.2 of Question.1

# Repeating Question.1 of (1) for N = 1,00,000 times
...
# Type I error means we reject the null hypothesis when it is true. It is also known as 'False Positive'. The significance level or alpha (a) is the probability of making a type I error. We usually set it as 0.05 or 0.01.
...
# The difference between 3(a) and 3 (b) is that - in 3 (a), though the probability is p = 0.5, for the N = 40, the number of A and B are not the same. For my case , A = 18 and B = 22. But, in 3 (b) we used random permutated block randomization with a block size of 40. And for this case number of A and B is same. For my case , A = 20 and B = 20.
...
# View the resulting treatment assignments by block. There are 5 different blocks with a block size of 8
